...
def llm_translate(text, language: str):
    ...
    # 将列表中的中文翻译成{language}，翻译后的结果按照顺序返回，只需返回翻译后的列表即可,列表里的内容请使用双引号，不要使用单引号，不需要其他任何额外的内容。
    # 需要翻译的内容如下：
    # {text}
    # """
    image_parse_prompt = f"""
请将以下 ERP 系统文本从中文翻译为{language}，确保使用专业术语并符合软件界面的语言习惯。 ,翻译后根据原文检查是否有问题，没问题后再返回，只需返回翻译后的结果即可,不需要其他任何额外的内容。
待翻译的内容:
{text}
"""
    ...
    try:
        ...
        print(f'text_generation exception, {text}', e)
        return None
